...
const UUID = process.env.UUID || '30aa6e46-b873-475d-8fc3-7bc03599e7b0';  // 在不同的平台运行了v1哪吒请修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'teelu.6is.dpdns.org';           // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiNDEyZTU1YjcxMWIyOTNiZmViMTNlYzlhODZiMzA1MjYiLCJ0IjoiMjcyNDk1MDEtMjQ1Ny00Y2U0LThhODctMjA0MGQzZmFkNTJhIiwicyI6Ik5UbGxNMlZoTldRdE5UZzRaUzAwWVRNNExUbGxaVEF0WVdZeU9XUmtZekJpTWpWaCJ9';               // argo固定隧道token或json,留空即使用临时隧道
const ARGO_PORT = process.env.ARGO_PORT || 8001;             // argo固定隧道端口,使用token需在cloudflare控制台设置和这里一致，否则节点不通
const S5_PORT = process.env.S5_PORT || '5368';                   // socks5端口，支持多端口的可以填写，否则留空
const TUIC_PORT = process.env.TUIC_PORT || '';               // tuic端口，支持多端口的可以填写，否则留空
const HY2_PORT = process.env.HY2_PORT || '5368';                 // hy2端口，支持多端口的可以填写，否则留空
